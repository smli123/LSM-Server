...
public class IRQueryIRDataHandle implements ICallFunction{
	/**********************************************************************************************************
	...
	{	
		...
		try
		{
			...
			if (i_irdata_type == IR_DATA_AIRCON_ALL_NAME) {			// 获取所有的空调名字
				String strIRList = ServerWorkThread.getIRSet();
			...
			} else if (i_irdata_type == IR_DATA_AIRCON_SAVE_NAME) {	// 保存空调名字
				...
				//写数据库
				boolean bRet = dbMgr.UpdateModuleInfo_AirCon(strModuleId, strAirConName);
				if(!bRet)
				{
					dbMgr.Rollback();
			...
			} else if (i_irdata_type == IR_DATA_AIRCON_LOAD_NAME) {	// 获取空调名字
				String strAirConName = dbMgr.getIRName(strModuleId);
