...
public class DebugUDPWorkThread  implements Runnable{
	private DatagramPacket debugPacket = null;
	private ServerDBMgr m_dbMgr = new ServerDBMgr();
	private static int DEBUG_SERVER_BUFFER_SIZE = 128;
	
	DebugUDPWorkThread(DatagramPacket debug_socket)
	{
		debugPacket = debug_socket;
	}
	private void SignalParse(String strRecord) throws ClassNotFoundException, SQLException, IOException
	{
		...
		{
			Response(String.format("Invalid debug cmd(%s)\n", strRecord).getBytes());
		}
		m_dbMgr.Destroy();
	}
	...
	private void DeleteUser(String strRecord) throws SQLException, IOException
	{
		...
		{
			Response(String.format("Failed to delete user %s", strUserName).getBytes());
		}
		Response(String.format("Succeed to delete user %s", strUserName).getBytes());
	}
	private void ShowUserInfo() throws ClassNotFoundException, SQLException, IOException
	{
		...
		Response(strUserInfoList.getBytes());
	}
	private void ShowModuleInfo() throws ClassNotFoundException, SQLException, IOException
	{
		...
		{
			Response(String.format("no device exist.").getBytes());
			return;
		}
		...
		Response(strModuleInfoList.getBytes());
	}
	...
	private void DebugHelpHandle() throws IOException
	{
		String strHelp = "=============help cmd list[UDP]================\n"
		                        ...
		                        + "ShowOnlineModule \n";
		Response(strHelp.getBytes());
	}
	
	
	public void run() {
		// TODO Auto-generated method stub
		LogWriter.WriteTraceLog(LogWriter.SRV_SELF_LOG,String.format("Debug Client[UDP] %s connectd.",debugPacket.getSocketAddress().toString()));
		/*STEP1 解释字符串*/
		try {
				byte[] buffer = new byte[DEBUG_SERVER_BUFFER_SIZE];
			
				if(null == debugPacket.getData())
				{
					LogWriter.WriteDebugLog(LogWriter.SRV_SELF_LOG,String.format("Debug Request(%s) be closed.",debugPacket.getAddress().toString()));				   
					return;
				}
				String strRecord = StringConvertTool.Byte2String(debugPacket.getData());
				LogWriter.WriteDebugLog(LogWriter.SRV_SELF_LOG,String.format("DebugUDPWorkThread Receive Debug Commond(%s)", strRecord));
		...
		LogWriter.WriteTraceLog(LogWriter.SRV_SELF_LOG,String.format("Debug Client[UDP] %s quit.",debugPacket.getSocketAddress().toString()));
	}

	public void Response(byte[] info)
	{
		try {
			DatagramPacket dataPacket = new DatagramPacket(info, info.length, debugPacket.getAddress(), debugPacket.getPort());    
			DebugUDPServerThread.dataSocket.send(dataPacket);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			LogWriter.WriteExceptionLog(LogWriter.SRV_SELF_LOG,e);
